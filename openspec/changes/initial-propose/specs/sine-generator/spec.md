...
The system SHALL accept four parameters via standard library `argparse`: frequency (`--frecuencia`), amplitude (`--amplitud`), line color (`--color`), and an optional PNG export flag (`--exportar`).

#### Scenario: User provides all arguments via CLI
- **WHEN** the user executes the command with `--frecuencia 5.0 --amplitud 2.0 --color red --exportar`
...
- **THEN** the CLI displays parameter descriptions and usage guidance
